import { createClaudeAnalyzer } from './claude-factory';
...
export function getAnalysisProviderFromEnv(): AnalysisProvider {
  const rawProvider =
    process.env['ANALYSIS_AI_PROVIDER'] ?? process.env['ANALYSIS_PROVIDER'] ?? 'openai';
  return rawProvider.toLowerCase() === 'openai' ? 'openai' : 'claude';
}

export function createAnalysisEngineFromEnv(deps: AnalysisFactoryDeps = {}): AnalysisEngine {
