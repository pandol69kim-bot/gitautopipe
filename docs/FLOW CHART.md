...
flowchart TB
    ...
    CLI --> Security{보안 검사}
    Security -- 실패 --> AuditFail[감사 로그 기록 후 종료]
...
```mermaid
flowchart LR
    MissionFile[Mission 파일 변경] --> Event[mission:updated 이벤트 발행]
    Event --> Orchestrator[워크플로우 오케스트레이터]
    Orchestrator --> Step1[Step1: mission-log\nMission 내용 수집]
    Step1 --> Step2[Step2: claude-analyze\nClaude AI 분석]
    Step2 --> Step3[Step3: linkedin-draft\nLinkedIn 초안 생성]
    Step3 --> Done([완료])
```
...
```mermaid
flowchart LR
    NotionDB[(Notion DB)] --> Step1[Step1: notion-fetch\n미팅 데이터 조회]
    Step1 --> Step2[Step2: obsidian-sync\nObsidian 볼트 동기화]
    Step2 --> Step3[Step3: report-update\nAnalysis 보고서 업데이트]
    Step3 --> Done([완료])
```
...
```mermaid
flowchart LR
    SkillFile[Skill 파일 변경] --> Event[skill:updated 이벤트 발행]
    Event --> Step1[Step1: site-build\n정적 사이트 빌드]
    Step1 --> Step2[Step2: vercel-deploy\nVercel 배포]
    Step2 --> Step3[Step3: deploy-notify\n배포 알림 전송]
    Step3 --> Done([완료])
```
...
```mermaid
flowchart LR
    Cron([cron: 0 9 * * 1]) --> Step1[Step1: vault-scan\n볼트 주간 데이터 수집]
    Step1 --> Step2[Step2: weekly-report\n주간 보고서 생성]
    Step2 --> Step3[Step3: github-commit\nGitHub 커밋]
    Step3 --> Step4[Step4: digest-notify\n다이제스트 알림 전송]
    Step4 --> Done([완료])
```
...
flowchart TB
    ...
    Handler -- error --> Fail
    Handler -- success --> AuditOK[감사 로그 success 기록]
    Fail --> AuditFail[감사 로그 failure 기록]
```
...
flowchart TB
    ...
    Result -- 실패 --> Policy{errorHandling.strategy}
