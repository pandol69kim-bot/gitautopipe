...
class ReportGenerator {
    ...
    // ── Subtask 4: 멤버별 보고서 ──────────────────────────────────────
    async generateMemberReport(memberId, contributions) {
    ...
    // ── Subtask 5: 팀 전체 개요 ──────────────────────────────────────
    async generateTeamOverview(history) {
        const trends = await this.analyzer.identifyTrends(history);
        const avgParticipation = history.reduce((sum, h) => sum + (h.participationRate ?? 0), 0) / history.length;
    ...
    // ── 파일명 생성 헬퍼 ──────────────────────────────────────────────
    generateFileName(type, weekNumber, memberId) {
