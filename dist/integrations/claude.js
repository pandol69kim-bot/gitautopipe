...
const ClaudeConfigSchema = zod_1.z.object({
    apiKey: zod_1.z.string().min(1, 'apiKey는 필수입니다'),
    model: zod_1.z.string().default(DEFAULT_MODEL),
    maxTokens: zod_1.z.number().int().positive().default(DEFAULT_MAX_TOKENS),
    maxRetries: zod_1.z.number().int().min(0).default(DEFAULT_MAX_RETRIES),
    retryDelayMs: zod_1.z.number().int().positive().default(DEFAULT_RETRY_DELAY_MS),
});
// ── 4.3 프롬프트 템플릿 ───────────────────────────────────────────────
const PROMPTS = {
...
class ClaudeAnalyzer {
    ...
    async callWithRetry(prompt, maxTokens) {
        let lastError = null;
        ...
        throw new Error(`Claude API 호출 실패 (${this.config.maxRetries}회 재시도): ${lastError?.message}`);
    }
    // ── 헬퍼 메서드 ──────────────────────────────────────────────────────
    ...
    extractBulletPoints(markdown, sectionTitle) {
    ...
    deduplicateKeywords(keywords) {
        ...
        return Array.from(map.values()).sort((a, b) => b.relevance - a.relevance || b.frequency - a.frequency);
    }
    sleep(ms) {
