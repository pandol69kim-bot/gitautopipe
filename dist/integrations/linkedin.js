...
class LinkedInContentGenerator {
    ...
    // ── Subtask 4: 해시태그 추가 ──────────────────────────────────────
    async addHashtags(content, keywords) {
    ...
    async formatForPlatform(post, mission) {
        ...
        return {
            content,
            charCount,
            isWithinLimit: charCount <= linkedin_1.LINKEDIN_CHAR_LIMIT,
            hashtags: post.hashtags,
            fileName: this.buildFileName(mission),
        };
    }
    // ── static 헬퍼: 해시태그 빌드 ───────────────────────────────────
    static buildHashtags(keywords) {
        const seen = new Set();
        const result = [];
...
exports.LinkedInContentGenerator = LinkedInContentGenerator;
