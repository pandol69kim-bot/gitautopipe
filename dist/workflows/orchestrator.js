...
const website_deployer_1 = require("./website-deployer");
...
class WorkflowOrchestrator {
    ...
    async runDueSchedules(at = new Date()) {
        const dueSchedules = this.getSchedules().filter((schedule) => (0, cron_1.isCronDue)(schedule.cron, at));
        const executions = [];
    ...
    createMissionCollectStep() {
        return {
            ...
            execute: async (ctx) => {
                ...
                if (!mission) {
                    const skipped = { status: 'skipped', reason: 'mission-not-found' };
                    ctx.payload = {
                        ...ctx.payload,
                        missionUpdate: {
                            ...(missionUpdate ?? {}),
    ...
    createMissionAnalysisStep() {
        return {
            ...
            execute: async (ctx) => {
                ...
                fs.writeFileSync(outputPath, this.buildMissionAnalysisMarkdown(mission, summary, keywords, trends), 'utf-8');
                const result = {
                    status: 'generated',
                    provider: 'openai',
                    missionTitle: mission.title,
    ...
    createMissionLinkedInDraftStep() {
        return {
            id: 'linkedin-draft',
            name: 'LinkedIn 초안 생성',
            execute: async (ctx) => {
    ...
    createSkillVercelDeployStep() {
        return {
            ...
            execute: async (ctx) => {
                ...
                const deploymentStatus = await deployer.waitForDeploymentReady(deployment.deploymentId, this.resolveDeploymentPollingOptions());
                ...
                if (status !== 'completed') {
                    throw new Error(deploymentStatus.errorMessage ?? `배포가 준비 완료 상태가 아닙니다: ${finalDeployment.state}`);
                }
        ...
        return {
            ...
            execute: async (ctx) => {
                ...
                if (!deployment?.deploymentId || !deployment.url || deployment.status !== 'completed') {
                    const skipped = { status: 'skipped', reason: 'deployment-not-completed' };
                    ctx.payload = {
                        ...ctx.payload,
                        skillUpdate: {
                            ...(skillUpdate ?? {}),
                ...
                if (!process.env['NOTIFICATION_WEBHOOK_URL']) {
                    const skipped = { status: 'skipped', reason: 'notification-webhook-not-configured' };
                    ctx.payload = {
                        ...ctx.payload,
                        skillUpdate: {
                            ...(skillUpdate ?? {}),
    ...
    async collectLatestMissionData(scanner, payload) {
        const files = await scanner.scanFolder('mission');
        if (files.length === 0) {
            return null;
        }
        const requestedFilePath = typeof payload?.['filePath'] === 'string' ? path.resolve(String(payload['filePath'])) : null;
    ...
    createMissionOpenAIAnalysisEngine() {
    ...
    resolveWebsiteDeploySourceFolder() {
    ...
    async verifyDeploymentAccess(deployer, deployment, status) {
        ...
        const verification = await deployer.verifyDeploymentUrl(verificationUrl, this.resolveDeploymentVerificationOptions());
    ...
    mergeDeploymentResult(deployment, deploymentStatus) {
        return {
            ...deployment,
    ...
    mapDeploymentStateToCommandStatus(state) {
        switch (state) {
            case 'INITIALIZING':
                return 'initializing';
            case 'READY':
    ...
    mapVerificationStatus(verification) {
        if (!verification.reachable) {
            return 'unreachable';
        }
        return verification.accessControlled ? 'access-controlled' : 'reachable';
    }
    parsePositiveIntEnv(key, fallback) {
    ...
    buildMissionAnalysisMarkdown(mission, summary, keywords, trends) {
        const lines = [
            '---',
            ...
            '',
            ...(keywords.length > 0
                ? keywords.map((item) => `- ${item.keyword} (빈도: ${item.frequency}, 관련성: ${item.relevance.toFixed(2)})`)
                : ['- 키워드 없음']),
            '',
            '## 트렌드',
            '',
    ...
    readMissionUpdatePayload(payload) {
    ...
    readSkillUpdatePayload(payload) {
